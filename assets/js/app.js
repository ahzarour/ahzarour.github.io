// --- Utilities ---
    ...
    function updateClassification() {
      ...
      document.getElementById("whiteClassSummary").textContent = `Hot ≥ ${whiteCls.hotMin
        } • Cold ≤ ${whiteCls.coldMax}`;
      document.getElementById("redClassSummary").textContent = `Hot ≥ ${redCls.hotMin
        } • Cold ≤ ${redCls.coldMax}`;
      renderBoard(
      ...
      if (typeof window.renderHistory === 'function' && document.getElementById('historyDataContainer')?.style.display !== 'none') {
    ...
    function applyDateFilterAndRecompute() {
      ...
      if (drawsEl2)
        drawsEl2.textContent = totalDraws ? totalDraws.toLocaleString() : "—";
      if (drawsEl3)
        drawsEl3.textContent = totalDraws ? totalDraws.toLocaleString() + ' Draws' : "—";
      if (drawsEl4)
        drawsEl4.textContent = totalDraws ? totalDraws.toLocaleString() + ' Draws' : "—";
    ...
    function evaluateTicketQuality(n1, n2, n3, n4, n5, pb) {
      ...
      // Range alignment
      const stats = computeRangeGroups(state.draws || []);
      const getTop2 = (arr) => arr.map((v, i) => ({ v, i })).sort((a, b) => b.v - a.v).slice(0, 2).map(x => x.i);
      ...
      const b1 = binIndex(user_n1), b2 = binIndex(user_n2);
      const b3 = binIndex(user_n3), b4 = binIndex(user_n4);
    ...
    (function attachGroupTicketChecker() {
      ...
      function renderGroupTicketResults() {
        ...
        for (const res of groupTicketResults) {
          ...
          if (res.error) {
            ...
            ballsDiv.innerHTML = res.whites.map(w => `<div class="ball ${state.classes?.white?.cls?.[w] || 'natural'}">${w}</div>`).join('') +
      ...
      btn.addEventListener('click', () => {
        ...
        for (let line of lines) {
          ...
          if (!res || res.error) {
            groupTicketResults.push({ line, error: true, errorMsg: res ? res.error : 'Data not loaded' });
          } else {
            res.line = line;
    ...
    (function attachCombinationChecker() {
      ...
      btn.addEventListener('click', () => {
        ...
        const matchCounts = {
          '5 Numbers': { draws: 0, combos: 0 }, '5 Numbers + PB': { draws: 0, combos: 0 },
          '4 Numbers': { draws: 0, combos: 0 }, '4 Numbers + PB': { draws: 0, combos: 0 },
          '3 Numbers': { draws: 0, combos: 0 }, '3 Numbers + PB': { draws: 0, combos: 0 },
          '2 Numbers': { draws: 0, combos: 0 }, '2 Numbers + PB': { draws: 0, combos: 0 },
        };
      ...
      function renderComboDetailList(container, matches, matchKey) {
        ...
        const sortedHeader = container.querySelector(`.combo-header [data-sort-key="${comboDetailSort.key}"]`);
        if (sortedHeader) {
          sortedHeader.innerHTML += comboDetailSort.dir === 'asc' ? ' <span style="font-size:0.8em">▲</span>' : ' <span style="font-size:0.8em">▼</span>';
        }
      }
    })();
    ...
    (function attachHistoryViewer() {
      ...
      if (headerStrength) {
        headerStrength.addEventListener('click', () => {
          ...
          else { sortBy = 'strength'; sortDir = 'desc'; }
          renderHistory();
        });
      }
      ...
      function renderHistory() {
        ...
        if (!state.draws || !state.draws.length) {
          list.innerHTML = '<div style="padding:10px; text-align:center; color:var(--muted);">No data loaded.</div>';
          return;
        }
        ...
        const drawsWithScores = state.draws.map(d => {
          const res = evaluateTicketQuality(d.whites[0], d.whites[1], d.whites[2], d.whites[3], d.whites[4], d.red);
          if (res && !res.error) {
        ...
        if (currentStrengthFilters.size > 0) {
          filteredDraws = drawsWithScores.filter(d => d.res && !d.res.error && currentStrengthFilters.has(d.res.grade.label));
        }
        ...
        if (headerDate) headerDate.innerHTML = `Date <span style="font-size:0.8em">${sortBy === 'date' ? (sortDir === 'asc' ? '▲' : '▼') : ''}</span>`;
        if (headerStrength) headerStrength.innerHTML = `Strength <span style="font-size:0.8em">${sortBy === 'strength' ? (sortDir === 'asc' ? '▲' : '▼') : ''}</span>`;
